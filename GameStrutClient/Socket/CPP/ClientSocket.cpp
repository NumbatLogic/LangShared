...
namespace NumbatLogic
{
    ...
    bool ClientSocket::Send(Blob* pBlob)
    {
        ...
        if (m_nWriteDataSize + pBlob->GetSize() > m_nWriteBufferSize)
        {
            unsigned int newSize = m_nWriteBufferSize * 2;
            while (newSize < m_nWriteDataSize + pBlob->GetSize())
                newSize *= 2;
    ...
    Blob* ClientSocket::Receive()
    {
        ...
        pBlob->Resize(m_nReadDataSize, false);
        pBlob->PackData(m_pReadBuffer, 0, m_nReadDataSize);
